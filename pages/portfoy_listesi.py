...
@st.dialog("Fotoğraf Galerisi", width="large")
def foto_galeri_dialog(baslik, foto_urls_json, startkey_url, portfoy_id):
    """Portföy fotoğraf galerisi — modal slider."""
    import json as _json

    try:
        fotolar = _json.loads(foto_urls_json) if isinstance(foto_urls_json, str) else (foto_urls_json or [])
    except Exception:
        fotolar = []

    # Parametresiz URL'leri öne al (tam boyut)
    fotolar_sorted = [f for f in fotolar if "?" not in f] + [f for f in fotolar if "?" in f]
    fotolar = fotolar_sorted if fotolar_sorted else fotolar

    if not fotolar:
        st.warning("Bu ilan için fotoğraf bulunamadı.")
        if startkey_url:
            st.link_button("Startkey'de Gör", url=startkey_url, use_container_width=True)
        return

    # Başlık + meta
    st.markdown(
        f'<div style="background:var(--secondary-background-color,#f8fafc);'
        f'border-left:3px solid #1E3A5F;padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:14px;">'
        f'<div style="font-size:14px;font-weight:700;color:#0f172a;line-height:1.3;">{_html.escape(baslik)}</div>'
        f'<div style="font-size:11px;color:#64748b;margin-top:3px;">'
        f'{len(fotolar)} fotoğraf · Portföy #{_html.escape(str(portfoy_id))}</div>'
        f'</div>',
        unsafe_allow_html=True
    )

    # Ana slider — components.html ile JS powered
    foto_json_str = _json.dumps(fotolar, ensure_ascii=False)
    components.html(f"""
<style>
*{{box-sizing:border-box;margin:0;padding:0;font-family:system-ui,sans-serif}}
#slider-wrap{{position:relative;width:100%;background:#0f172a;border-radius:10px;overflow:hidden}}
#main-img{{width:100%;height:340px;object-fit:contain;display:block;background:#0f172a}}
#img-placeholder{{width:100%;height:340px;display:flex;align-items:center;justify-content:center;
  color:#475569;font-size:14px;background:#0f172a}}
.arrow{{position:absolute;top:50%;transform:translateY(-50%);
  width:38px;height:38px;border-radius:50%;background:rgba(0,0,0,.55);
  border:none;color:#fff;font-size:20px;cursor:pointer;display:flex;
  align-items:center;justify-content:center;z-index:10;transition:background .15s}}
.arrow:hover{{background:rgba(0,0,0,.8)}}
#arr-l{{left:10px}} #arr-r{{right:10px}}
#counter{{position:absolute;bottom:10px;left:50%;transform:translateX(-50%);
  background:rgba(0,0,0,.55);color:#fff;font-size:11px;font-weight:600;
  padding:3px 12px;border-radius:999px}}
#thumbs{{display:grid;grid-template-columns:repeat(6,1fr);gap:5px;margin-top:8px}}
.thumb{{width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:5px;
  cursor:pointer;opacity:.6;transition:opacity .15s;border:2px solid transparent}}
.thumb.active{{opacity:1;border-color:#1E3A5F}}
.thumb:hover{{opacity:.9}}
</style>
<div id="slider-wrap">
  <img id="main-img" src="" alt="" style="display:none" />
  <div id="img-placeholder">Yükleniyor...</div>
  <button class="arrow" id="arr-l">&#8249;</button>
  <button class="arrow" id="arr-r">&#8250;</button>
  <div id="counter">1 / {len(fotolar)}</div>
</div>
<div id="thumbs"></div>
<script>
const FOTOLAR = {foto_json_str};
let cur = 0;
...
function goTo(i) {{
  cur = (i + FOTOLAR.length) % FOTOLAR.length;
  mainImg.style.display = 'none';
  placeholder.style.display = 'flex';
  placeholder.textContent = 'Yükleniyor...';
  const img = new Image();
  img.onload = () => {{
    mainImg.src = FOTOLAR[cur];
    mainImg.style.display = 'block';
    placeholder.style.display = 'none';
  }};
  img.onerror = () => {{ placeholder.textContent = 'Görsel yüklenemedi'; }};
  img.src = FOTOLAR[cur];
  counter.textContent = (cur+1) + ' / ' + FOTOLAR.length;
  document.querySelectorAll('.thumb').forEach((t,idx) => {{
    t.classList.toggle('active', idx === cur);
  }});
  // Aktif thumb'ı görünür yap
  const at = thumbsDiv.children[cur];
  if(at) at.scrollIntoView({{block:'nearest',inline:'center'}});
}}

// Thumbnail'leri oluştur
FOTOLAR.forEach((url, i) => {{
  const img = document.createElement('img');
  img.src = url;
  img.className = 'thumb' + (i===0 ? ' active' : '');
  img.onclick = () => goTo(i);
  img.onerror = () => {{ img.style.background='#1e293b'; img.style.minHeight='48px'; }};
  thumbsDiv.appendChild(img);
}});

document.getElementById('arr-l').onclick = () => goTo(cur-1);
document.getElementById('arr-r').onclick = () => goTo(cur+1);
...
""", height=440, scrolling=False)
    ...
    # Alt butonlar
    fb1, fb2, fb3 = st.columns([1, 1.5, 1])
    with fb1:
        st.caption(f"{len(fotolar)} fotoğraf")
    with fb2:
        if st.button("✨ Sunum Merkezi'ne Al", key="fdlg_sunum", use_container_width=True, type="primary"):
            st.session_state["sunum_portfoy_url"] = startkey_url
            st.session_state["sunum_portfoy_id"]  = portfoy_id
            st.success("Sunum Merkezi'ne aktarıldı!")
    with fb3:
        if startkey_url:
            st.link_button("Startkey'de Gör →", url=startkey_url, use_container_width=True)
...
gorunum = st.session_state.get("zil_gorunum", "kart")
    ...
    for i in range(0, len(liste_df), 2):
        ...
        for j, col in enumerate(cols2):
            ...
            with col:
                ...
                foto_urls_raw = row.get("foto_urls","") or "[]"
                foto_count_lbl = ""
                try:
                    import json as _jj
                    _fc = len(_jj.loads(foto_urls_raw) if isinstance(foto_urls_raw,str) else foto_urls_raw)
                    foto_count_lbl = f"📷 {_fc}" if _fc > 0 else "📷"
                except Exception:
                    foto_count_lbl = "📷"
                ...
                with _k2:
                    if st.button(foto_count_lbl, key=f"pkart_foto_{i}_{j}",
                                 use_container_width=True, help="Fotoğraf galerisini aç"):
                        baslik_val = baslik_formatla(str(row.get("baslik","") or ""))
                        foto_galeri_dialog(
                            baslik      = baslik_val,
                            foto_urls_json = foto_urls_raw,
                            startkey_url   = link,
...
# ── GÖRÜNÜM: TABLO ───────────────────────────────────────────────────────────
    ...
    if "Danışman" in tablo_df.columns:
        # HTML tag içeriyorsa temizle
        tablo_df["Danışman"] = tablo_df["Danışman"].apply(
            lambda v: re.sub(r"<[^>]+>", "", str(v)).strip() if v and "<" in str(v) else (str(v) if v else "—")
        )
    if "Brüt m²" in tablo_df.columns:
        tablo_df["Brüt m²"] = tablo_df["Brüt m²"].apply(
            lambda v: str(v).strip() if v and str(v) not in ("nan","None","") else ""
        )
    if "Net m²" in tablo_df.columns:
        tablo_df["Net m²"] = tablo_df["Net m²"].apply(
            lambda v: str(v).strip() if v and str(v) not in ("nan","None","") else ""
        )
