...
if st.session_state.get("pft_hizli", "Son 3 ay") not in allowed_hizli:
    st.session_state["pft_hizli"] = "Son 3 ay"

# hizli global scope'da tanımla — portfoy_karti içinden erişilebilsin
hizli = st.session_state.get("pft_hizli", "Son 3 ay")

son_3ay_sayisi  = sum(1 for v in veriler if tarih_gun_farki(en_iyi_tarih(v)) <= 90)
son_6ay_sayisi  = sum(1 for v in veriler if tarih_gun_farki(en_iyi_tarih(v)) <= 180)
son_1yil_sayisi = sum(1 for v in veriler if tarih_gun_farki(en_iyi_tarih(v)) <= 365)
...
with _hdr1:
    render_page_header("📦 Portföy Arşivi", f"45 günden eski kayıtlar · {len(veriler)} portföy · {danisman_sayisi} danışman")
with _hdr2:
...
st.markdown(
    ...
    '📦 <b>Arşiv görünümü</b> — 45 günden eski portföyler. Aktif kayıtlar için '
    '<a href="/3_Portfoy_Tablosu" style="color:#92400e;font-weight:700;">Portföy Havuzu</a>\'nu kullanın.'
    '</div>',
    unsafe_allow_html=True
)
...
if not veriler:
    ...
    st.info(f"{kaynak_et} kaynağında arşivde portföy bulunamadı.")
    st.stop()

hizli_sayilar = {
    "Tümü":      len(veriler),
    "Son 3 ay":  son_3ay_sayisi,
    "Son 6 ay":  son_6ay_sayisi,
    "Son 1 yıl": son_1yil_sayisi,
}
...
if hizli != "Tümü":
    gl = {"Son 3 ay": 90, "Son 6 ay": 180, "Son 1 yıl": 365}.get(hizli, 9999)
...
if gorunum == "Tablo":
    ...
    if _aktif_ana == "Favorilerim" and _aktif_sub not in ("İzmir İlçeleri","Diğer İlçeler"):
        ...
        if not favori_f:
            donem = {"Tümü": "tüm zamanlarda", "Son 3 ay": "son 3 ayda", "Son 6 ay": "son 6 ayda", "Son 1 yıl": "son 1 yılda"}.get(
                st.session_state.get("pft_hizli","Son 3 ay"), "bu dönemde")
