...
with st.expander("+ Yeni Senaryo Oluştur", expanded=True):
    with st.form("dp_senaryo_form", clear_on_submit=True):
        ...
        st.markdown("**Değerleme Raporu Sonuçları** (opsiyonel)")
        d1, d2, d3, d4 = st.columns(4)
        with d1:
            f_kisa = st.text_input("Kısa Vade (TL)", key="dp_sn_kisa", placeholder="örn. 16000000")
        with d2:
            f_orta = st.text_input("Önerilen/Orta Vade (TL)", key="dp_sn_orta", placeholder="örn. 17500000")
        with d3:
