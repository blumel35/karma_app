...
# ─────────────────────────────────────────
# MERKEZİ TABLO (izmir_pazar_ilanlar) — bu sayfa zaten tüm markaları
# çekiyor, Startkey'e filtrelemeden önce ATMAK yerine bu ortak tabloya
# yazıyoruz. Böylece FSBO ve Pazar Radar sayfaları aynı veriyi kendi Revy
# sorgularını atmadan, sadece marka filtresiyle okuyabiliyor. Pazar Raporu
# KAPSAM DIŞI — kendi derin/bölgesel sorgusuyla çalışmaya devam ediyor.
#
# DÜZELTME (2026-08-30): Yardımcı fonksiyonlar (_mt_* ve
# merkezi_tabloya_yaz) artık BURADA tanımlı değil — core/izmir_pazar_sync.py'ye
# taşındı, çünkü aynı mantığa yeni bir headless GitHub Actions işi
# (izmir_pazar_sync_job.py, günlük otomatik senkronizasyon) de ihtiyaç
# duyuyor. İki ayrı kopya yerine tek kaynak; davranış birebir aynı.
# ─────────────────────────────────────────
from core.izmir_pazar_sync import merkezi_tabloya_yaz
...
r3a, r3b, r3c, r3d, r3e, r3f, r3g = st.columns([1, 1, 1, 1, 1.1, 1.1, 0.8])
...
with r3e:
    kullanim_filtre = st.multiselect(
        "Kullanım",
        ["Boş", "Kiracılı", "Mülk Sahibi"],
        placeholder="Tümü",
        key="sk_kullanim"
    )
