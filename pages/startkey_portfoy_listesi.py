...
def rehber_yukle():
    ...
    try:
        ...
        supa = get_client()
        ofis_res = supa.table("rehber_ofisler").select("ofis_adi,telefon,mail").eq("aktif", True).execute()
...
def _norm(s):
    """Fuzzy match için metni normalize et."""
    s = str(s).lower().strip()
    s = re.sub(r"[^a-z0-9ğüşıöçİĞÜŞÖÇ ]", " ", s)
    for old, new in [("startkey","sk"),("gayrimenkul","gm"),("emlak",""),
                     ("danışmanlık",""),("ltd",""),("şti",""),("sti","")]:
        s = s.replace(old, new)
    return re.sub(r"\s+", " ", s).strip()

@st.cache_data
def ofis_eslesme_haritasi(revy_ofis_listesi: tuple, rehber_ofis_listesi: tuple):
    """Revy ofis adlarını rehber ofis adlarına eşleştir — fuzzy match."""
    harita = {}
    for revy_ad in revy_ofis_listesi:
        if not revy_ad or str(revy_ad).strip() in ("", "nan"): continue
        norm_revy = _norm(revy_ad)
        en_iyi = difflib.get_close_matches(norm_revy,
            [_norm(r) for r in rehber_ofis_listesi], n=1, cutoff=0.55)
        if en_iyi:
            idx = [_norm(r) for r in rehber_ofis_listesi].index(en_iyi[0])
...
if st.session_state.sk_cache_zaman:
    st.markdown(f'<div class="cache-bilgi">📦 Kayıtlı veri: <b>{st.session_state.sk_cache_zaman}</b> tarihli — yeni veri için Listele\'ye basın</div>', unsafe_allow_html=True)
...
if listele_btn:
    if not secili_durumlar:
        ...
        filtre_dict = {
            ...
            "baslangic": bas_tarih.strftime("%Y-%m-%d") if bas_tarih else None,
            "bitis":     bit_tarih.strftime("%Y-%m-%d") if bit_tarih else None,
        }
        ...
        try:
            ...
            cache_kaydet(cikti)  # dosyaya kaydet
...
# ── Rehber yükle + ofis adı normalize et ──
...
if not rehber_ofis_df.empty and "Ofis" in df_ham.columns:
    ...
    harita = ofis_eslesme_haritasi(revy_ofis_listesi, rehber_ofis_listesi)
    df_ham["Ofis_norm"] = df_ham["Ofis"].map(harita).fillna(df_ham["Ofis"])
else:
    df_ham["Ofis_norm"] = df_ham.get("Ofis", "")

# ── SEKMELER ──
sekme1, sekme2 = st.tabs(["📋 İlan Listesi", "👤 Danışman Bazlı"])
    ...
    if "Ofis" in df.columns:
        ...
        with oc2:
            ...
            fig = px.bar(top15, x="Kayıt", y="Ofis", orientation="h",
                color="Ofis", color_discrete_sequence=SK_RENKLER[:len(top15)], text="Kayıt")
...
rows = ""
for i,(_, row) in enumerate(tablo.iterrows(), 1):
...
html_t = ("<style>.ht tbody tr:hover td{background:#EEF4FA!important;}</style>"
    ...
    +"</div></div>")
components.html(html_t, height=min(1200, 80+len(tablo)*52), scrolling=True)
