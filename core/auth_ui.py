# core/auth_ui.py
# Hesap Aktivasyonu (davet) ekranının render mantığı.
#
# DÜZELTME (2026-08-27): Bu mantık önceden pages/Hesap_Aktivasyon.py içinde
# top-level kod olarak duruyordu ve app.py, davet linkinin kök URL'de
# yakalandığı anda st.switch_page() ile bu sayfaya GEÇİŞ yapıyordu. Üç ayrı
# denemede switch_page güvenilmez çıktı: (1) StreamlitAPIException
# "Could not find page" — st.navigation() henüz oturmadan string yol ile
# çağrıldığında; (2) tarayıcı seviyesinde "Page not found" ekranı — sayfa
# geçişi sırasında URL, Streamlit'in içeride beklediği urlPathname ile tam
# eşleşmediğinde; (3) query params (token_hash) hedef sayfanın ilk
# render'ında kayboluyordu. Kök sebep hep aynıydı: SAYFA GEÇİŞİ kırılgan.
#
# Şimdiki çözüm: bu render mantığı bağımsız bir fonksiyona taşındı.
# app.py, davet parametrelerini kök URL'de gördüğü anda HİÇ SAYFA
# GEÇİŞİ YAPMADAN bu fonksiyonu doğrudan çağırıyor — aynı script çalışması
# içinde olduğu için query params hiç kaybolmuyor, tarayıcı URL'i hiç
# değişmiyor (kök URL'de kalıyor), switch_page hiç devreye girmiyor.
# pages/Hesap_Aktivasyon.py hâlâ duruyor ve bu fonksiyonu çağırıyor —
# biri doğrudan o sayfanın kendi URL'sine giderse diye.
...
def render_hesap_aktivasyon() -> None:
    # ─────────────────────────────────────────────────────
