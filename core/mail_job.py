...
def run_full_mail_job(ai_enabled=True, ai_limit=20, durum_callback=None):
    ...
    Faz 2.7 (22.09.2026 — Meltem: "su an karma app aktif kullanmıyorum
    localde var bu işlemi githu otomatik mail çekimi gibi ekleyemez
    miyiz"): AI adımından önce reset_basarisiz_kayitlar() de çağrılıyor.
    Böylece uygulamayı hiç açmasa/kullanmasa bile, AI kredisi/bakiyesi
    bir süreliğine biterse "failed" kalan kayıtlar, kredi geri
    yüklendiğinde bir SONRAKİ otomatik (30 dakikalık) GitHub Actions
    çalıştırmasında kendiliğinden 'raw'a çevrilip tekrar denenir — manuel
    müdahale (Supabase'e SQL yazmak veya Mail İşlem sayfasındaki butona
    basmak) GEREKMEZ. reset_basarisiz_kayitlar() SADECE kredi hatasının
    imzasıyla ("BadRequestError: Error code: 400") başarısız olmuş
    kayıtlara dokunur, başka sebeplerden 'failed' olanlara dokunmaz — bu
    yüzden her çalıştırmada çağrılması güvenli, gereksiz kayıt
    "kurtarmaz".
    """
    ...
    parse_sonuc = None
    if ai_enabled:
        reset_basarisiz_kayitlar(durum_callback=durum_callback)
